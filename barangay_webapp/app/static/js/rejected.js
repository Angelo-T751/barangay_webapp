document.addEventListener('DOMContentLoaded', () => {
    // Shared applicants data
    let allApplicants = JSON.parse(localStorage.getItem('allApplicants')) || [
        { id: "01", name: "Juan Dela Cruz", type: "Barangay Clearance", purpose: "Verification of requirements...", date: "05/08/26" },
        { id: "02", name: "Jean Valjean", type: "Business Permit", purpose: "Verification of requirements...", date: "05/08/26" },
        { id: "03", name: "Maria Clara", type: "Indigency Certificate", purpose: "Verification of requirements...", date: "05/08/26" }
    ];
    ...
    function renderTable(searchTerm = "") {
        ...
        filtered.forEach(app => {
            ...
            row.innerHTML = `
                <div>${app.id}</div>
                <div style="font-weight: 800; color: #dc2626;">${app.name}</div>
                <div>${app.type}</div>
    ...
    window.validateAndUpdate = () => {
        if(selectedStatus === "Rejected") {
            triggerError("Please select a new status (Pending or Approved)!");
            return;
        }
        ...
        // SYNC PIE CHART & DASHBOARD CARDS
